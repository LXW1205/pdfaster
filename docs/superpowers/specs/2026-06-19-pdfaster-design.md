...
- **Date:** 2026-06-19 (v1 design) · 2026-06-20 (phase 7 — polish + remaining features) · 2026-06-20 (phase 8 — quality-of-life polish) · 2026-06-20 (phase 9 — final QoL) · 2026-06-21 (phase 10 — Extract tool) · 2026-06-22 (phase 11 — select + move + resize) · 2026-06-22 (phase 12 — measure-once thumb height + infinite-scroll pagination) · 2026-06-22 (phase 13 — preview pages on the 3 organize tools + Move tool polish + Shift precision)
- **Status:** v1 shipped · phase 7 closed the polish + remaining-features phase · phase 8 ships the QoL polish · phase 9 closes the final-QoL batch · phase 10 adds the **Extract** tool (non-contiguous page selection) · phase 11 adds **select + move + resize** for all annotations · phase 12 ships the **measure-once thumb height** (fixes the Letter-aspect overlap) and **infinite-scroll pagination** for the 3 tool pages with checkboxes/lists (Reorder, Delete, Extract) · phase 13 adds **page previews** to the 3 organize tools, renames the **Select** tool to **Move**, adds **Shift-to-fine-move/resize** (¼ speed), and fixes a pre-existing **move-drift bug** in the move handler
...
## Phase 13 polish (shipped 2026-06-22)

- **Page previews on the 3 organize tools.** Each row in Reorder, Delete pages, and Extract now renders a small 60px-wide page thumbnail at the left. The preview is a `<canvas>` painted by pdf.js (same worker the editor uses); the canvas's height is derived from the page's aspect ratio at width=60, so the row's height is stable even before the canvas paints. The preview is intentionally quiet (gray border, no shadow) — it supports the row's text labels, doesn't compete with them. The preview's `dpr=1` (waste-bytes argument: these are tiny thumbnails, the extra backing-store bytes aren't worth the marginal crispness). *ponytail: the preview is a quiet thumbnail, not a hero image.* The user sees a brief "no preview" state (rows render without a preview) for the few hundred ms it takes pdf.js to load the proxy; once the proxy is ready, every loaded row's preview paints.
- **`usePdfDocument` hook.** New `src/lib/hooks/use-pdf-document.ts` (~25 lines) — one `PDFDocumentProxy` per file, shared across every row's preview. Dynamic import of `lib/pdf-render.ts` keeps pdf.js out of the bundle of any tool page that doesn't import it. The hook returns `null` until the proxy loads; consumers render a placeholder (or just skip the preview) in the meantime. *ponytail: re-creating the proxy on every render would be the natural bug. The hook is the only "advanced-init-once" surface for the previews — see vercel-react-best-practices.* The `cancelled` flag in the effect tears down an in-flight load when the file changes; the resulting proxy is GC'd when no reference holds it.
- **`PagePreview` component.** New `src/components/PagePreview.tsx` (~55 lines) — a single-page pdf.js render. The `useEffect` resolves the page, sets the canvas's CSS size + backing store size, renders, and calls `page.cleanup()`. Cancels on unmount or `pageIndex` change. `width` is configurable (default 60). The `data-testid="page-preview-{i}"` is the e2e anchor.
- **"Move" rename.** The `select` tool (the no-op default, activated by the `V` shortcut) is now visible in the toolbar as a "Move" button. The underlying ToolId stays `select` — the keyboard map (`V`), the state, the ARIA semantics all reference `select`. Only the visible label is "Move", which matches the user's mental model: this tool moves existing annotations. The button lives at the start of the ToolPicker (before the annotation tools), styled identically. *ponytail: a UI label change is a 1-line PR; renaming the state would be a 20-file refactor. The label-vs-ID separation is the load-bearing design choice.*
- **Shift-to-fine-move (¼ speed).** Holding Shift during a move drag divides the CSS-px delta by 4 — the standard "precision mode" pattern in design tools (Figma, Sketch, Photoshop all use ~1/4 as the slow-mode step). The same factor applies to **resize** in `SelectionHandles` — Shift during a resize drag = ¼ size step. The factor is computed in CSS pixels (where `shiftKey` is checked) and only then converted to PDF points via `/zoom`, so the user-visible "1 pixel = 1/4 pixel" math is what they feel. *ponytail: ¼ is a common default; ⅕ or ⅛ are also common in some tools. The factor is a single literal — bump to 0.125 if ¼ feels too coarse, document the choice.*
- **Move-drift bug fix.** The pre-existing move handler in `AnnotationOverlay` computed the new rect as `r.x + dxPts` (where `r` is the CURRENT rect from the store, already moved by previous pointermoves in the same drag). With `steps: 8` in Playwright (or 60+ Hz in a real browser), each pointermove compounded the delta, so the annotation drifted far past the cursor. The select-and-move e2e (phase 11) only checks the quadrant, so it didn't catch this. The fix: capture the annotation's `rect` / `points` at pointerdown in `moveRef.originalRect` / `originalPoints`, and compute the new state as `original + delta`. The cumulative-delta intent (see the existing "do NOT update `m.startCssX/Y`" comment) is now actually implemented. The resize handler in `SelectionHandles` already used the correct `startRect` pattern — only the move handler was affected.
- **Cheatsheet updated.** The `V` entry changed from "Select tool" to "Move / select" (the visible affordance is "Move"; the underlying state is "select"). A new entry `{ keys: ['Shift', 'drag'], desc: 'Fine move (¼ speed)' }` documents the precision modifier. The cheatsheet's `?` shortcut, the `Esc` close, and the toolbar's data-driven tool shortcuts are unchanged.
- **31st and 32nd e2e tests.** `e2e/preview-pages.spec.ts` drops a 3-page fixture into `/tools/reorder`, asserts the list is visible, and asserts every row's `[data-testid="page-preview-N"]` canvas has non-zero width and height (5s timeout for the proxy load). `e2e/shift-fine-move.spec.ts` draws a highlight, drags it 200 CSS px normally (1:1 speed), then drags it 200 CSS px with Shift (¼ speed), exports, re-opens via inspect, and asserts the final highlight x is in a band that proves Shift was honored (200 < x < 400 — the un-Shifted total would be ~480, the broken-Shift total would be far larger). The assertion is approximate; the contract is "Shift = ¼ speed", not "Shift = exactly ¼ × N CSS px".
...
- **Playwright e2e (32 tests, all green)** — load sample PDF → draw highlight + free-draw → fill a form field → draw a signature → export → re-open exported in pdf.js → assert annotations present + form value persisted. Plus: worker URL assertion, session restore, a11y smoke (landing/editor/merge page), and the round-trip checks for each tool page. Phase 10 adds two Extract tests (checkbox-list happy path + range-input short path). Phase 11 adds the select-and-move test (the inspect payload is extended with `annotationRects: { pageIndex, id, type, rect: {x,y,w,h} }[]` so a moved highlight's /Rect can be asserted to live in the bottom-right quadrant of the page). Phase 12 adds the infinite-scroll test (drops a 30-page fixture into `/tools/reorder`, asserts 20 rows initially, scrolls, asserts 30 rows after the observer fires). Phase 13 adds the preview-pages test (3-page fixture on Reorder, asserts every row's `[data-testid="page-preview-N"]` canvas is visible with non-zero size) and the shift-fine-move test (draws a highlight, drags it 200 CSS px normally then 200 CSS px with Shift, exports, asserts the final x is in a band that proves Shift = ¼ speed).
